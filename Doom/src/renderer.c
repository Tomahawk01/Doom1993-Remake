...
const char* fragSrc =
	...
	"  } else if (TexType == 1) {\n"
	"    color = vec3(texelFetch(u_pallete, int(texture(u_flat_tex, vec3(TexCoords, TexIndex)).r), 0));\n"
	"  } else if (TexType == 2) {\n"
	"    color = vec3(texelFetch(u_pallete, int(texture(u_wall_tex, vec3(fract(TexCoords / MaxTexCoords) * MaxTexCoords, TexIndex)).r), 0));\n"
	"  }\n"
